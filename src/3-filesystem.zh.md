...
然后是`next`方法。 它返回一个`Option`- 就像一个迭代器，当它返回`None`时，迭代器结束。这返回的类型是一个`Result`是因为`read_line`可能会失败，我们永远不要错失错误。 所以如果失败了，我们把它的错误包进`Some<Result>`。 否则，文件的自然结束时，它可能读取到零字节 - 而零字节不是错误，只是一个`None`。
...
而且，生命周期的这个签名不兼容`Iterator`的接口(/trait)， 但是如果兼容就很容易出现问题; 考虑到`collect`试图制作这些字符串切片的 Vec，可这是不能工作的，因为它们都是从同一个可变字符串中借用的! (如果您已 读取了文件的 _所有_ ，并转换为字符串，而这个字符串的`lines`迭代器是可以返回字符串切片，因为它们都是借用原始字符串的 _不同_ 部分)。
...
这很诱人，但你在这里抛出一个可能的错误;每当发生错误时，此循环都会静静地停止。 特别是，它将停止在，无法将 行 转换为 utf-8 的第一处位置。适合休闲代码，不适合生产代码!
...
一个`PathBuf`就好像`String`- 它拥有一组可扩展的角色方法，但具有专门用于构建路径的方法。 但其大部分功能都来自借用版本的`Path`，这就像`&str`。 所以，举个例子，`is_dir`就是一个`Path`方法。
